package com.educat.android.educatapp.services;
...
//import com.educat.android.educatapp.communication.GroupedTcpTelegram;
...
import com.educat.android.educatapp.activities.MainActivity;
import com.educat.android.educatapp.communication.Connectivity;
import com.educat.android.educatapp.communication.FT311UARTInterface;
import com.educat.android.educatapp.communication.StreamData;
import com.educat.android.educatapp.communication.TcpTelegram;
import com.educat.android.educatapp.communication.UsbTelegram;
import com.educat.android.educatapp.helperClasses.AppController;
import com.educat.android.educatapp.helperClasses.ChannelIds;
...
public class UsbAndTcpService extends Service {
    ...
    private static int androidAsInstrumentFirstValueIndex = -1;
...
//    private GroupedTcpTelegram groupedTcpTelegram = new GroupedTcpTelegram();
...
//                                    MyLog.d("CycleCounterDebug", "Cycle counter rec: " + cycleCounter);
                                    ...
                                    if (cycleCounter % 100 == 0){
                                        MyLog.d(TAG, "Telegram " + cycleCounter + ": " + Utilities.bytesToHex(usbTelegramBytes));
...
//                                            MyLog.d("CycleCounterDebug", "USB Cycle Counter: " + streamData.getCycleCounter());
                                            ...
                                            byte[] androidBytes = prepareAndroidInstrumentData(cycleCounter, bigDecimalArrayList);
                                            ...
                                            if (measurementRunning){
                                                ...
                                                if (androidAsInstrumentDataIndex >= 0 && androidBytes != null){
                                                    switch (androidAsInstrumentOutputDataType) {
                                                        case (int) Constants.SETUP_PRM_DATA_OUTPUT_DATATYPE_option_USB_AD_AS_INSTRUMENT_0XF1:
                                                            System.arraycopy(androidBytes, 4, tcpDu, androidAsInstrumentDataIndex + 4, 2);
                                                            break;

                                                        case (int) Constants.SETUP_PRM_DATA_OUTPUT_DATATYPE_option_USB_AD_AS_INSTRUMENT__SENSOR_ACTIVATE_BITS_0XF2:
                                                            System.arraycopy(androidBytes, 4, tcpDu, androidAsInstrumentDataIndex + 4, 3);
                                                            break;
                                                    }
                                                }
...
//                                                            if (groupedTcpTelegram.addTcpTelegram(tcpTelegram) == 5) {
//                                                                mManager.send(groupedTcpTelegram);
//                                                                groupedTcpTelegram = new GroupedTcpTelegram();
//                                                            }
...
//            MyLog.e(TAG, "NACK " + shortFromByteArray(cycleCounter));
     ...
     *
     * It also checks whether or not Android as instrument is in the setup.
     */
            ...
            if (oasFound){
                for (Parameter parameter : instrument.getParameterArrayList()){
                    switch (parameter.getId()){
                        case (int) Constants.SETUP_PRM_INSTRUMENT_1_ID:
                        case (int) Constants.SETUP_PRM_INSTRUMENT_2_ID:
                        case (int) Constants.SETUP_PRM_INSTRUMENT_3_ID:
                        case (int) Constants.SETUP_PRM_INSTRUMENT_4_ID:
                        case (int) Constants.SETUP_PRM_INSTRUMENT_5_ID:
                        case (int) Constants.SETUP_PRM_INSTRUMENT_6_ID:
                        case (int) Constants.SETUP_PRM_INSTRUMENT_7_ID:
                        case (int) Constants.SETUP_PRM_INSTRUMENT_8_ID:
                            distanceInstrumentIds.add(parameter.getValue());
                            break;

                        case (int) Constants.SETUP_PRM_OAS_SLOPE_START:
                            slopeStart = parameter.getValue();
                            break;

                        case (int) Constants.SETUP_PRM_OAS_SLOPE_PERCENTAGE:
                            slopePercentage = parameter.getValue();
                            break;

                        case (int) Constants.SETUP_PRM_OAS_SLOPE_END:
...
//        MyLog.d(TAG, "distanceInstrumentIds.size(): " + distanceInstrumentIds.size());
        ...
        for (Instrument instrument : streamSetup.getInstrumentArrayList()){
                ...
                if (f.intValue() == instrument.getId()){
                    ...
                    for (Parameter parameter : instrument.getParameterArrayList()){
                        ...
                        switch (parameter.getId()){
                            ...
                            case (int) Constants.SETUP_PRM_DISTANCE_CALIBRATION:
                                boundaryCalibration = parameter.getValue();
                                break;
                        }
                    }

                    oasRadarSegments.add(new RadarSegment(x, y, r, instrument.getId(), instrument.getVariableArrayList().get(0).getValueIndex(), boundaryCalibration));
            ...
            // Search for Android as instrument
            if (instrument.getOutputDataType() == (int) Constants.SETUP_PRM_DATA_OUTPUT_DATATYPE_option_USB_AD_AS_INSTRUMENT_0XF1 || instrument.getOutputDataType() == (int) Constants.SETUP_PRM_DATA_OUTPUT_DATATYPE_option_USB_AD_AS_INSTRUMENT__SENSOR_ACTIVATE_BITS_0XF2){
                androidAsInstrumentOutputDataType = instrument.getOutputDataType();
                androidAsInstrumentFirstValueIndex = instrument.getVariableArrayList().get(0).getValueIndex();
            }
        }

        int count = oasRadarSegments.size();

//        MyLog.d(TAG, "oasRadarSegments.size(): " + count);

        sensorsStatus = (byte) 0x00;
        ...
        if (oasRadarSegments.size() > 0){
            oasInitialized = true;
        }
    }

    /**
     * Calculates and prepares the "Android as instrument"-data from the OAS.
     *
     * @param bigDecimalArrayList containing the measured distances as floating points
     */
    private byte[] prepareAndroidInstrumentData(long cycleCounter, ArrayList<BigDecimal> bigDecimalArrayList){
        ...
        BigDecimal bigDecimalSpeed;
        float currentSpeed = 0f;

        bigDecimalSpeed = bigDecimalArrayList.get(bigDecimalArrayListSize - actualSpeedValueIndex);
        if (bigDecimalSpeed != null){
            currentSpeed = bigDecimalSpeed.floatValue();
        }
        ...
        for (int i = oasRadarSegments.size() - 1; i >= 0; i--){

            index = bigDecimalArrayListSize - oasRadarSegments.get(i).getValueIndex();

            if (index >= 0){
                bigDecimal = bigDecimalArrayList.get(index);
                if (bigDecimal != null){
                    currentDistance = bigDecimal.floatValue() - oasRadarSegments.get(i).getBoundaryCalibration();
                }
                else {
                    currentDistance = 0.0f;
                }

                if (currentDistance != 0.0f && (sensorsStatus >> i & 1) == 1){
                    shortestDistance = Math.min(shortestDistance, currentDistance);
                }
            }
        }

        activateAlert = shortestDistance < alarmDistance;

        // Enable or disable buzzer on Android device
        if (dataStreamActive && activateAlert && defaultSharedPreferences.getBoolean(Constants.SETTING_OAS_BUZZER, false)){
            if (!buzzerActive){
        ...
        switch (androidAsInstrumentOutputDataType) {
            case (int) Constants.SETUP_PRM_DATA_OUTPUT_DATATYPE_option_USB_AD_AS_INSTRUMENT_0XF1:
                ...
                // Prepare DU for "Android device as instrument"-message
                byte[] sixBytes = new byte[6];

                System.arraycopy(Utilities.longToByteArray(cycleCounter), 4, sixBytes, 0, 4);

                if (activateAlert){
                    sixBytes[4] = (byte) 0xFF;
                }

                if (defaultSharedPreferences.getBoolean(Constants.SETTING_OAS_BUZZER, false)){
                    sixBytes[5] |= 1;
                }

                if (defaultSharedPreferences.getBoolean(Constants.SETTING_OAS_HAPTIC, false)){
                    sixBytes[5] |= 1 << 1;
                }

                if (defaultSharedPreferences.getBoolean(Constants.SETTING_OAS_VISUAL, false)){
                    sixBytes[5] |= 1 << 2;
                }

                if (cycleCounter % 100 == 0){
                    if (defaultSharedPreferences.getBoolean(Constants.SETTING_SHOW_DETAILED_DATA, false)){
                        localLog(TAG, "S: Android as instrument sent | Cycle counter: " + cycleCounter + " | Data: " + Utilities.bytesToHex(sixBytes));
                    }
                    else {
                        localLog(TAG, "S: Android as instrument sent | Cycle counter: " + cycleCounter);
                    }
                }

                sendAndroidInstrumentData(sixBytes);

                index = bigDecimalArrayListSize - androidAsInstrumentFirstValueIndex;
                if (androidAsInstrumentFirstValueIndex > 0 && index > 0){
                    bigDecimalArrayList.set(index, new BigDecimal(Integer.toString(Utilities.byteToUnsignedInt((byte) (sixBytes[4] & 0xFF)))));
                    index--;
                    bigDecimalArrayList.set(index, new BigDecimal(Integer.toString(Utilities.byteToUnsignedInt((byte) (sixBytes[5] & 0xFF)))));
                }

                return sixBytes;

            case (int) Constants.SETUP_PRM_DATA_OUTPUT_DATATYPE_option_USB_AD_AS_INSTRUMENT__SENSOR_ACTIVATE_BITS_0XF2:
                // Prepare DU for "Android device as instrument with sensor activate bits"-message
                byte[] sevenBytes = new byte[7];

                System.arraycopy(Utilities.longToByteArray(cycleCounter), 4, sevenBytes, 0, 4);

                if (activateAlert){
                    sevenBytes[4] = (byte) 0xFF;
                }
                ...
                sevenBytes[6] = sensorsStatus;

                if (cycleCounter % 100 == 0){
                    if (defaultSharedPreferences.getBoolean(Constants.SETTING_SHOW_DETAILED_DATA, false)){
                        localLog(TAG, "S: Android as instrument sent | Cycle counter: " + cycleCounter + " | Data: " + Utilities.bytesToHex(sevenBytes));
                    }
                    else {
                        localLog(TAG, "S: Android as instrument sent | Cycle counter: " + cycleCounter);
                    }
                ...
                sendAndroidInstrumentData(sevenBytes);

                index = bigDecimalArrayListSize - androidAsInstrumentFirstValueIndex;
                if (androidAsInstrumentFirstValueIndex > 0 && index > 0){
                    bigDecimalArrayList.set(index, new BigDecimal(Integer.toString(Utilities.byteToUnsignedInt((byte) (sevenBytes[4] & 0xFF)))));
                    index--;
                    bigDecimalArrayList.set(index, new BigDecimal(Integer.toString(Utilities.byteToUnsignedInt((byte) (sevenBytes[5] & 0xFF)))));
                    index--;
                    bigDecimalArrayList.set(index, new BigDecimal(Integer.toString(Utilities.byteToUnsignedInt((byte) (sevenBytes[6] & 0xFF)))));
                }
        ...
        for (int i = streamSetup.getInstrumentArrayList().size() - 1; i >= 0; i--){
            ...
            if (instrument.getVariableArrayList().size() > 0){
                ...
                if (status == (byte) 0x80){
                    ...
                    if (instrument.getOutputDataType() == (int) Constants.SETUP_PRM_DATA_OUTPUT_DATATYPE_option_USB_AD_AS_INSTRUMENT_0XF1){
                        androidAsInstrumentDataIndex = index - 2;
                    }

                    if (instrument.getOutputDataType() == (int) Constants.SETUP_PRM_DATA_OUTPUT_DATATYPE_option_USB_AD_AS_INSTRUMENT__SENSOR_ACTIVATE_BITS_0XF2) {
                        androidAsInstrumentDataIndex = index - 3;
                    }

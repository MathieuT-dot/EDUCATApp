...
import com.educat.android.educatapp.helperClasses.HexadecimalInputFilter;
    ...
    private void displayParameters() {
        ...
        for (int i = 0; i < parameterCount; i++)
        {
            ...
            parameterInfoImageViews[i].setOnClickListener(v -> {
                ...
                if (parameter.getId() == Constants.SETUP_PRM_OAS_SLOPE_START || parameter.getId() == Constants.SETUP_PRM_OAS_SLOPE_PERCENTAGE || parameter.getId() == Constants.SETUP_PRM_OAS_SLOPE_END) {
                    ImageView imageView = parameterInfoView.findViewById(R.id.slope_image_view);
                    imageView.setVisibility(View.VISIBLE);

                    Button button = parameterInfoView.findViewById(R.id.simulation_button);
                    button.setVisibility(View.VISIBLE);
                    button.setOnClickListener(v1 -> {
                        Intent intent = new Intent(context, AlarmDistanceSimulationActivity.class);
    ...
    private void initD4ArrayLists() {
        ...
        for (Instrument currentInstrument : setup.getInstrumentArrayList()){
            if (currentInstrument.getId() != instrument.getId()){
                if (instrument.getInstrumentTypeId() == 15 || instrument.getInstrumentTypeId() == 24){
                    if (
                ...
                else if (instrument.getInstrumentTypeId() == 26) {
                    if (
                            currentInstrument.getInstrumentTypeId() == 10 ||
                                    currentInstrument.getInstrumentTypeId() == 19 ||
                                    currentInstrument.getInstrumentTypeId() == 22
                    ) {
                        d4SpinnerArrayList.add(currentInstrument.getId() + ": " + currentInstrument.getName());
                        d4IdArrayList.add(currentInstrument.getId());
                    }
